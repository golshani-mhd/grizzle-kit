...
func (g *Generator) generateEntityFile(entity EntityInfo) error {
	...
	file.Const().Id("TableName").Op("=").Lit(entity.Table.Name)
	for _, col := range entity.Columns {
		colVar := g.generateColumnVariable(col, entity.Table.Name)
		file.Add(colVar)
		file.Line()
	}
	file.Line()
...
func (g *Generator) generateColumnVariable(col ColumnInfo, tableName string) jen.Code {
	goName := g.toGoIdentifier(col.Name)
	columnStruct := jen.Values(jen.Dict{
		jen.Id("ParentAlias"):  jen.Lit(tableName),
	...
	if col.AutoIncrement {
		columnStruct = jen.Values(jen.Dict{
			...
			jen.Id("Type"):          jen.Qual("github.com/golshani-mhd/grizzle-kit/types", col.AbstractType).Dot("String").Call(),
			jen.Id("AbstractType"):  jen.Qual("github.com/golshani-mhd/grizzle-kit/types", col.AbstractType),
			jen.Id("AutoIncrement"): jen.Lit(col.AutoIncrement),
		})
	}
	if col.HasDefault || col.Length != nil || col.Precision != nil || col.Scale != nil {
		dict := jen.Dict{
			jen.Id("ParentAlias"):  jen.Lit(tableName),
			jen.Id("Name"):         jen.Lit(col.Name),
			jen.Id("Type"):         jen.Qual("github.com/golshani-mhd/grizzle-kit/types", col.AbstractType).Dot("String").Call(),
		...
		if col.AutoIncrement {
			dict[jen.Id("AutoIncrement")] = jen.Lit(col.AutoIncrement)
		}
		if col.HasDefault {
			dict[jen.Id("HasDefault")] = jen.Lit(col.HasDefault)
			dict[jen.Id("Default")] = g.generateDefaultValue(col.DefaultValue, col.GoType)
		}
		if col.Length != nil {
			dict[jen.Id("Length")] = jen.Op("&").Lit(*col.Length)
		}
		if col.Precision != nil {
			dict[jen.Id("Precision")] = jen.Op("&").Lit(*col.Precision)
		}
		if col.Scale != nil {
			dict[jen.Id("Scale")] = jen.Op("&").Lit(*col.Scale)
		}
		columnStruct = jen.Values(dict)
	}
	return jen.Var().Id(goName).Op("=").Qual("github.com/golshani-mhd/grizzle-kit/types", "Column").Index(jen.Id(col.GoType)).Add(columnStruct)
}
...
func (g *Generator) generateAsMethod(entity EntityInfo) jen.Code {
	...
	for _, col := range entity.Columns {
		goName := g.toGoIdentifier(col.Name)
		field := jen.Id(goName).Id("*").Qual("github.com/golshani-mhd/grizzle-kit/types", "Column").Index(jen.Id(col.GoType))
	...
	for _, col := range entity.Columns {
		goName := g.toGoIdentifier(col.Name)
		dict[jen.Id(goName)] = jen.Id(goName).Dot("WithAlias").Call(jen.Id("alias"))
	}
